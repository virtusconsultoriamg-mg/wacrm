...
import { ContactListsPanel } from '@/components/contacts/contact-lists-panel';
...
export default function ContactsPage() {
  ...
  const fetchContacts = useCallback(async () => {
    ...
    if (selectedListId && accountId) {
      const { data, error } = await supabase.rpc('filter_contacts_by_list', {
    ...
    setLoading(false);
  }, [supabase, page, search, selectedTagIds, selectedListId, tagsMap, t, accountId]);
  ...
  async function handleDeleteAll() {
    if (!accountId) return;
    setDeleting(true);
    const { data, error } = await supabase.rpc('delete_all_contacts_for_account', { p_account_id: accountId });
    if (error) toast.error(`Falha ao excluir contatos: ${error.message}`);
    else {
      toast.success(`${Number(data ?? 0)} contatos excluídos.`);
      setSelected(new Set());
  ...
  const hasActiveFilters = search.trim().length > 0 || selectedTagIds.length > 0 || !!selectedListId;
  ...
  return (
    <div className="space-y-6">
      ...
      <ContactListsPanel
        ...
        onSelect={(id) => { setSelectedListId(id); setPage(0); }}
      ...
      <Dialog open={deleteAllOpen} onOpenChange={setDeleteAllOpen}>
        <DialogContent className="bg-popover border-border text-popover-foreground sm:max-w-md">
          <DialogHeader>
            <DialogTitle>Excluir todos os contatos?</DialogTitle>
            <DialogDescription>Esta ação remove todos os contatos da conta de uma vez. Conversas e mensagens vinculadas aos contatos também poderão ser removidas por cascata; disparos e negócios mantêm seus registros quando o banco permitir.</DialogDescription>
          </DialogHeader>
          <DialogFooter>
            ...
            <Button variant="destructive" onClick={() => void handleDeleteAll()} disabled={deleting}>
              {deleting && <Loader2 className="size-4 animate-spin" />} Excluir todos
            </Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
