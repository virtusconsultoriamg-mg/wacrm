...
export interface AudienceConfig {
  type: 'all' | 'list' | 'tags' | 'custom_field' | 'csv';
  listId?: string;
  tagIds?: string[];
...
export function useBroadcastSending(): UseBroadcastSendingReturn {
  ...
  async function resolveAudience(audience: AudienceConfig): Promise<Contact[]> {
    ...
    if (audience.type === 'all') {
      ...
      if (listError) throw new Error(`Failed to fetch list contacts: ${listError.message}`);
      const ids = [...new Set((members ?? []).map((m) => m.contact_id))];
      if (ids.length > 0) {
        const { data, error } = await supabase.from('contacts').select('*').in('id', ids);
        if (error) throw new Error(`Failed to fetch list contacts: ${error.message}`);
        contacts = data ?? [];
      }
    } else if (
      audience.type === 'tags' &&
