...
type AudienceType = 'all' | 'list' | 'tags' | 'custom_field' | 'csv';
...
export function Step2SelectAudience({
  ...
  const audienceOptions = useMemo<{
    ...
    },
    {
      ...
      description: 'Envie somente para uma lista importada ou criada no CRM.',
      icon: Users,
    },
    {
      type: 'tags',
  ...
  useEffect(() => {
    async function fetchTags() {
      ...
      try {
        ...
        const [{ data: tagData }, { data: listData }] = await Promise.all([
    ...
    <div className="space-y-6">
      ...
      {audience.type === 'list' && (
        <div className="rounded-xl border border-border bg-card/50 p-4">
          <p className="mb-3 text-sm font-medium text-foreground">Selecione a lista</p>
          {loadingLists ? <Loader2 className="h-5 w-5 animate-spin text-primary" /> : lists.length === 0 ? (
            <p className="text-xs text-muted-foreground">Nenhuma lista criada em Contatos.</p>
          ) : (
            <div className="grid gap-2 sm:grid-cols-2">
              {lists.map((list) => (
                <button key={list.id} onClick={() => onUpdate({ ...audience, listId: list.id })} className={`rounded-lg border p-3 text-left ${audience.listId === list.id ? 'border-primary bg-primary/5' : 'border-border'}`}>
                  <p className="text-sm font-medium text-foreground">{list.name}</p>
                  <p className="text-xs text-muted-foreground">Usar esta lista como público</p>
                </button>
              ))}
            </div>
          )}
