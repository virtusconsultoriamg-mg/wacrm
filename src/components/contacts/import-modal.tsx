...
import { parseContactFile } from '@/lib/contacts/parse-contact-file';
import {
...
export function ImportModal({
  ...
  async function handleFileChange(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    let parsed;
    try {
      parsed = await parseContactFile(selected);
    } catch (error) {
      toast.error(
        error instanceof Error ? error.message : t('toastNoValidRows')
      );
      setParsedRows([]);
      return;
    }
    ...
    try {
      ...
      for (const r of existingRows ?? []) {
        ...
        if (row.phone_normalized)
          existingByPhone.set(row.phone_normalized, row.id);
      }
      const toInsert = unique.filter((row) => {
        const existingId = existingByPhone.get(normalizeKey(row.phone));
        if (existingId) {
          skipped++;
          resolvedContactIds.push(existingId);
          return false;
        }
        return true;
      });
        ...
        if (error) {
          ...
          for (let j = 0; j < rows.length; j++) {
            ...
            if (!singleErr && singleData) {
              imported++;
              resolvedContactIds.push(singleData.id);
          ...
          resolvedContactIds.push(...inserted.map((row) => row.id));
      ...
      let tagsAssigned = 0;
      if (targetListId && resolvedContactIds.length > 0) {
        const uniqueIds = [...new Set(resolvedContactIds)];
        const memberships = uniqueIds.map((contact_id) => ({
        ...
        if (listError)
          toast.warning(
            `Contatos importados, mas não foi possível vinculá-los à lista: ${listError.message}`
          );
      }
      try {
        ...
        <div className="border-border/80 shrink-0 space-y-4 border-b px-6 pt-6 pb-5">
          <DialogHeader className="gap-1.5">
            <DialogTitle className="text-popover-foreground text-lg">
              {targetListId ? 'Importar contatos para esta lista' : t('title')}
            </DialogTitle>
            <DialogDescription
