...
## Local VIRTUS enhancements — 2026-09-02

### Added
- Contact lists with account-scoped list membership, list tabs in Contacts, and list-based broadcast audiences.
- Importing contacts into the currently selected list, including existing contacts that were already in the CRM.
- One-click admin deletion of all contacts for the current account.
- AI media library for PDF, image and video assets.
- PDF ingestion into the existing AI knowledge base using the configured OpenAI key.
- AI auto-reply media command protocol (`[SEND_MEDIA:<asset-id>]`) for approved image/video assets, sent through WhatsApp using private Supabase Storage signed URLs.

### Migration
- Apply `supabase/migrations/040_contact_lists_and_ai_assets.sql` before using these features.
